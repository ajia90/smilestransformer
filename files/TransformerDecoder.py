...
class TransformerDecoderLayer(Module):
    ...
    def forward(self, memory: Tensor, memory_mask: Optional[Tensor] = None,
                memory_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        Shape:
            see the docs in Transformer class.
        """
        memory2 = self.self_attn(memory, memory, memory, attn_mask=memory_mask)[0]
        memory = memory + self.dropout1(memory2)
        memory = self.norm1(memory)
        memory2 = self.multihead_attn(memory, memory, memory, attn_mask=memory_mask)[0]
