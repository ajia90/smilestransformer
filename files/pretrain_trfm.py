...
class TrfmSeq2seq(nn.Module):
    ...
    def _decode(self, mem, tgt): 
        # src: (T,B)
        output = mem
        for i in range(self.trfm.decoder.num_layers):
            output = self.trfm.decoder.layers[i](output, tgt, None)  # (T,B,H)
    ...
    def decode(self, mem, tgt):
        ...
        print('There are {:d} molecules. It will take a little time.'.format(batch_size))
        st,ed = 0,100
        out = self._decode(mem[:,st:ed], tgt[:,st:ed]) # (B,4H)
        while ed<batch_size:
            st += 100
            ed += 100
            add = self._decode(mem[:,st:ed], tgt[:,st:ed])
            out = np.concatenate([out, add], axis=1)
...
def main():
    ...
    model.load_state_dict(torch.load('trfm_new_11_180000.pkl'))
    ...
    best_loss = None
    for e in range(12, args.n_epoch):
        for b, sm in tqdm(enumerate(train_loader)):
            ...
            if b%10000==0:
                ...
                if not best_loss or loss < best_loss:
                    print("[!] saving model...")
                    model_save_name = 'trfm_new_%d_%d.pkl' % (e,b)
