...
class TrfmSeq2seq(nn.Module):
    def __init__(self, in_size, hidden_size, out_size, n_layers, dropout=0.1):
        ...
        #num_encoder_layers=n_layers, num_decoder_layers=n_layers, dim_feedforward=hidden_size, custom_decoder = TransformerDecoder.TransformerDecoder)
        encoder_layer = TransformerEncoderLayer(hidden_size, 4, hidden_size, dropout)
        self.encoder = TransformerEncoder(encoder_layer, n_layers)
        decoder_layer = TransformerDecoderLayer(hidden_size, 4, hidden_size, dropout)
    ...
    def forward(self, src):
        ...
        #t = torch.zeros(src.shape).type_as(src.data)
        #tgt = self.embed(t)
        mask = generate_square_subsequent_mask(src.shape[0])
        #hidden = self.trfm(embedded, embedded, memory_mask=mask) # (T,B,H)
        hidden = self.encoder(embedded)
        hidden - self.decoder(hidden, hidden, mask)
    ...
    def _encode(self, src):
        ...
        for i in range(self.encoder.num_layers):
            output = self.encoder.layers[i](output, None)  # (T,B,H)
    ...
    def _decode(self, mem): 
        ...
        for i in range(self.decoder.num_layers):
            output = self.decoder.layers[i](mem, None)  # (T,B,H)
        if self.decoder.norm:
            output = self.decoder.norm(output) # (T,B,H)
        output = output.detach().numpy()
        return output


    def decode(self, mem):
        batch_size = mem.shape[1]
        if batch_size<=100:
            out = self._decode(mem)
        else: # Batch is too large to load
            print('There are {:d} molecules. It will take a little time.'.format(batch_size))
            st,ed = 0,100
            out = self._decode(mem[:,st:ed]) # (B,4H)
            while ed<batch_size:
                st += 100
                ed += 100
                add = self._decode(mem[:,st:ed])
                out = np.concatenate([out, add], axis=1)
        out = torch.from_numpy(out).float()
        output = self.out(out) # (T,B,V)
        output = F.log_softmax(output, dim=2)
        output = output.detach().numpy()
 
        #return np.hstack([np.mean(output, axis=0), np.max(output, axis=0), output[0,:,:] ]) # (B,4H)
        return output


def parse_arguments():
    parser = argparse.ArgumentParser(description='Hyperparams')
    parser.add_argument('--n_epoch', '-e', type=int, default=13, help='number of epochs')
    parser.add_argument('--vocab', '-v', type=str, default='vocab.pkl', help='vocabulary (.pkl)')
    parser.add_argument('--data', '-d', type=str, default='chembl_25.csv', help='train corpus (.csv)')
...
def main():
    ...
    model.load_state_dict(torch.load('trfm_notgt_9_0.pkl'))
        ...
        for b, sm in tqdm(enumerate(train_loader)):
            ...
            if b%10000==0:
                ...
                if not best_loss or loss < best_loss:
                    print("[!] saving model...")
                    model_save_name = 'trfm_notgt_%d_%d.pkl' % (e,b)
